...
///
/// \brief Transfers data stored in the bucket to the output arrays if there is
/// only one pass. Assumption: this bucket is filled to the end
///
/// \param[in] self the bucket
/// \param[in] count
/// \param[in] start the index where to start in the buffer
...
///
/// \brief Pushes an (i, x) pair into the buffer if there is only one pass.
///
...
///
/// \brief Flushes all buffer entries to the OUT arrays if there is only one
/// pass.
///
/// \param[in] buffer the buffer
/// \param[in] n_buckets the number of buckets
